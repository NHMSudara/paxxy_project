...
int bhi_flush_fifo(struct BHI_sensor *sensor, uint8_t fifo_type)
{
	uint8_t *rx;

	while(1==BHI_get_drdy_status(sensor))
	{
...
int bhi_read_fifo(struct BHI_sensor *sensor, uint8_t fifo_type, uint8_t *buffer)
{
	...
	else if(data_length.ui16 != 0)
	{
		//may have unread data flust the fifo;
		printf("%d:Invalid data length - %d\n", sensor->id, data_length.ui16);
		bhi_flush_fifo(sensor, fifo_type);
		return FAILED;
	}
	else
	{
...
//			printf("NON Wakup FIFO read %d\n", read_data_length);
    	...
    	if(0 != int_status._.status || 0 != int_status._.debug)
		{
