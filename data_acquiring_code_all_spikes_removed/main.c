...
{
	DataObject data;
	prossesDataObject process_data;

	if(YES==data_log_started)
	{
		if(YES==timer_ticked_ads)
		{
			process_data.current_ra = ads1298->adc_buffer[ads1298->adc_ri].channel[3];
			process_data.current_ll = ads1298->adc_buffer[ads1298->adc_ri].channel[4];
			process_data.current_la = ads1298->adc_buffer[ads1298->adc_ri].channel[5];
			process_data.current_v1 = ads1298->adc_buffer[ads1298->adc_ri].channel[7];

			if(ads1298->adc_ri==0)
			{
				process_data.last_ra = ads1298->adc_buffer[1].channel[3];
				process_data.last_ll = ads1298->adc_buffer[1].channel[4];
				process_data.last_la = ads1298->adc_buffer[1].channel[5];
				process_data.last_v1 = ads1298->adc_buffer[1].channel[7];
			}
			else
			{
				process_data.last_ra = ads1298->adc_buffer[0].channel[3];
				process_data.last_ll = ads1298->adc_buffer[0].channel[4];
				process_data.last_la = ads1298->adc_buffer[0].channel[5];
				process_data.last_v1 = ads1298->adc_buffer[0].channel[7];
			}
			
          	if(abs(process_data.current_ra - process_data.last_ra) > 12000){
				spike_remove_flag=YES;
			}
			if(abs(process_data.current_ll - process_data.last_ll) > 12000){
				spike_remove_flag=YES;
			}
			if(abs(process_data.current_la - process_data.last_la) > 12000){
				spike_remove_flag=YES;
			}
			if(abs(process_data.current_v1 - process_data.last_v1) > 12000){
				spike_remove_flag=YES;
			}
			
			if(spike_remove_flag==NO){
				fprintf(data_file,"%d,%d,%d,%d\n", ads1298->adc_buffer[ads1298->adc_ri].channel[3], ads1298->adc_buffer[ads1298->adc_ri].channel[4],
				 ads1298->adc_buffer[ads1298->adc_ri].channel[5], ads1298->adc_buffer[0].channel[7]);
				fflush(data_file);
