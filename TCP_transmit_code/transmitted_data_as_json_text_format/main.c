...
void log_data(FILE *data_file, struct ADS_sensor *ads1298, struct ADS_sensor *ads131, struct BHI_sensor *sensor1, struct BHI_sensor *sensor2, struct BHI_sensor *sensor3,
			  struct BHI_sensor *sensor4, struct BHI_sensor *sensor5, int sock)
{
	Tx_Data sample;
...
//	int8_t rslt=BHY2_OK;
	...
	while (1) //(count<30000)
	{
		...
		log_data(data_file, &ads1298, &ads131, &bhi_sensor1, &bhi_sensor2, &bhi_sensor3, &bhi_sensor4, &bhi_sensor5, sock);
#endif
